...
eRamRingBuffer::eRamRingBuffer(size_t capacity_bytes, size_t max_blocks) : m_max_blocks(max_blocks), m_write_offset(0), m_first_write_ms(0), m_block_write_idx(0), m_total_blocks(0) {
	...
	if (!m_buf || !m_blocks) {
		eWarning("[eRamRingBuffer] allocation failed (%zu MB) "
				 "- RAM timeshift disabled",
				 m_capacity >> 20);
		free(m_buf);
		free(m_blocks);
		m_buf = nullptr;
...
ssize_t eRamTsSource::read(off_t offset, void* buf, size_t count) {
	...
	if (offset < min_off) {
		...
		eDebug("[eRamTsSource] LAP pre-check: offset=%lld < min_off=%lld "
			   "-> lapped (aligned=%lld)",
	...
	if (rc < 0 && errno == EAGAIN) {
		...
		// min_off. Case 1 is the live edge: set m_exhausted so the watchdog
		// knows the buffer is truly drained.
