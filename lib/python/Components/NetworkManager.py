...
class Connection:
	...
	LAN adapters normally have one Connection.
	WLAN adapters can have many – one per known network – ordered by priority.
	"""
...
class Adapter:
	...
	Runtime state (kernel IP, up/down) lives here.
	All configuration lives in the connections list.
	"""
...
class NameserverConfig:
	...
	mode: "dhcp-router" | "custom" | "dnscrypt"
	"""
...
class InterfacesFile:
	...
	Enigma2-style commented-out disabled stanzas are handled transparently.
	"""
	...
	def parse(self) -> tuple[dict[str, list[Connection]], set[str]]:
		...
		Returns (connections_by_iface, auto_ifaces) where auto_ifaces is the set
		of interface names that have an uncommented "auto <iface>" line.
		conn.enabled reflects whether the iface stanza is uncommented (independent
		of the auto line).  adapter.adapterEnabled is set by the caller from auto_ifaces.
		"""
...
def _serialiseConnection(conn: Connection, adapterEnabled: bool) -> list[str]:
	...
	adapterEnabled  controls the "auto <iface>" line (adapter-level).
	conn.enabled    controls the "iface <iface>" stanza (connection-level).
	For LAN these are always the same; for WLAN they can differ independently.
	"""
	lines: list[str] = []
...
class NameserverFiles:
	...
	def load(self, ns: NameserverConfig) -> None:
	...
	def save(self, ns: NameserverConfig, anyDhcpActive: bool) -> None:
...
class NetEventReader:
	...
	structured event lines pushed by socketdaemon's monitor thread.
	Automatically reconnects when socketdaemon restarts.

	Event line format (ASCII, newline-terminated):
		UPDATE
		LINK,<iface>,up|down
	...
	def connectionLost(self, failure=None) -> None:
		self._disconnect()
...
class NetworkManager:
	...
	Replaces iNetwork (Components/Network.py) and the WirelessLan plugin.
	Use iNetworkManager singleton at module level.
	"""
	...
	@staticmethod
	def _detectWol(adapter: Adapter) -> None:
	...
	def _loadInterfacesFile(self) -> None:
	...
	def _loadWpaSupplicantFiles(self) -> None:
		for iface, adapter in self.adapters.items():
	...
	@staticmethod
	def _mergeWlanConfig(adapter: Adapter, wlan: WlanConfig) -> None:
	...
	def restartNetwork(self, iface: str = "", callback: Callable | None = None) -> None:
	...
	def addConnection(self, conn: Connection) -> None:
	...
	def setAdapterAttribute(self, iface: str, attr: str, value) -> None:
	...
	def getConfiguredAdapters(self) -> list[str]:
		...
		Returns interfaces that have at least one enabled Connection.
		"""
		return [
	...
	def ifaces(self) -> dict:
		...
		Builds a live snapshot: {iface: {"up", "ip", "netmask", "gateway", "mac", "dhcp"}}.
		"""
		result = {}
	...
	def deactivateNetworkConfig(self) -> None:
		"""Compatibility shim for old iNetwork.deactivateNetworkConfig() callers."""
		self.deactivateInterface(list(self.adapters.keys()))

	def _notifyNetworkPlugins(self, reason: bool) -> None:
		"""Fire WHERE_NETWORKCONFIG_READ plugins – but ONLY when at least one
		adapter is currently active (kernelUp + routable IP).

		reason=True  – config just written to disk; plugins may re-read state
		reason=False – network interface reload initiated; plugins should re-check

		The old Network.py called this blindly for every adapter regardless of
		whether it was connected.  We skip the call entirely when no adapter
		is active to avoid spurious wakeups for unconfigured interfaces.
		"""
		active = any(
	...
	def msgPlugins(self) -> None:
		"""Compatibility shim for old iNetwork.msgPlugins() callers."""
		self._notifyNetworkPlugins(True)

	def getLinkState(self, iface: str, callback) -> None:
		"""
		Compatibility shim for old iNetwork.getLinkState() callers.
		Reads physical link state from /var/run/netinfo (written by socketdaemon).
		Calls callback synchronously – no subprocess needed.
		"""
		try:
			info = _readNetinfo()
	...
	def checkNetworkState(self, callback) -> None:
		"""
		Compatibility shim for old iNetwork.checkNetworkState() callers.
		Pings three targets and calls callback(numPassed).
		"""
		try:
	...
	def writeNameserverConfig(self) -> None:
	...
	def getInterfaces(self, callback=None) -> None:
		"""
		Compatibility shim for old iNetwork.getInterfaces() callers.
		Re-discovers adapters and calls callback(True) when done.
		"""
		self._discoverAdapters()
		self._loadInterfacesFile()
		if callback:
	...
	def deactivateInterface(self, ifaces, callback=None) -> None:
	...
	def setBgscan(self, iface: str, bgscan: str) -> None:
	...
	def setRoamingMode(self, iface: str, mode: str) -> None:
	...
	def _updateWolPreup(self, adapter: Adapter, mode: str) -> None:
		conn = adapter.activeConnection()
		if conn is None:
			return
	...
	def _notifyAdaptersChanged(self) -> None:
	...
	def _onNetinfoUpdate(self) -> None:
		self._applyNetinfo()
		self._notifyAdaptersChanged()

	def _onLinkChange(self, iface: str, up: bool) -> None:
		adapter = self.adapters.get(iface)
		if adapter:
			adapter.kernelUp = up
	...
	def _onIpChange(self, iface: str, ipPrefix: str) -> None:
		adapter = self.adapters.get(iface)
		if adapter:
			adapter.kernelIp = _parseIp4(ipPrefix.split("/")[0])
		self._notifyAdaptersChanged()

	def checkConnectionInternet(self, callback: Callable[[dict[str, bool]], None]) -> None:
		"""Ping 8.8.8.8 (fallback 1.1.1.1) for each adapter that has physical link
		and a configured gateway (DHCP always qualifies; static only if gateway set).
		Calls callback({iface: ok}) on the enigma2 main thread when all checks finish.
		"""
		from twisted.internet import reactor
		...
		def _onResult(iface: str, ok: bool) -> None:
			results[iface] = ok
			remaining[0] -= 1
			if remaining[0] == 0:
				callback(results)

		def _check(iface: str) -> None:
			ok = _pingHost("8.8.8.8", iface) or _pingHost("1.1.1.1", iface)
			reactor.callFromThread(_onResult, iface, ok)

		for iface in candidates:
			reactor.callInThread(_check, iface)

	def _onIfaceAdd(self, iface: str) -> None:
		if iface not in self.adapters:
			self._discoverAdapters()
		self._notifyAdaptersChanged()

	def _onIfaceRemove(self, iface: str) -> None:
		self.adapters.pop(iface, None)
		self._notifyAdaptersChanged()

	def _onScanTrigger(self, iface: str) -> None:
...
def _pingHost(host: str, iface: str, timeout: float = 2.0) -> bool:
