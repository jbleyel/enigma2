...
class InfoBarSleepTimer:
	...
	def timerTimeout(self, action, timerMethod, name):
		...
		if action != "standby":
			...
			isJanitor = abs(self.session.nav.Janitor.getNextPowerManagerTime() - time()) <= 900 or self.session.nav.Janitor.isProcessing(exceptTimer=0)
			if isRecordTime or isPowerTime or isJanitor:
