...
eDVBServicePlay::eDVBServicePlay(const eServiceReference& ref, eDVBService* service, bool connect_event)
	: m_reference(ref), m_dvb_service(service), m_is_primary(1), m_decoder_index(0), m_have_video_pid(0), m_tune_state(-1), m_noaudio(false), m_is_stream(ref.path.find("://") != std::string::npos),
	  m_is_pvr(!ref.path.empty() && !m_is_stream), m_is_paused(0), m_timeshift_enabled(0), m_timeshift_active(0), m_timeshift_changed(0), m_save_timeshift(0), m_timeshift_fd(-1), m_skipmode(0),
	  m_fastforward(0), m_slowmotion(0), m_tap_recorder(0), m_cuesheet_changed(0), m_cutlist_enabled(1), m_subtitle_widget(0), m_subtitle_sync_timer(eTimer::create(eApp)),
	  m_nownext_timer(eTimer::create(eApp)), m_last_crypted_state_for_decoder(false), m_eof_recovery_timer(eTimer::create(eApp)), m_timeshift_delay_updater_timer(eTimer::create(eApp)),
	  m_saved_timeshift_delay(-1), m_stream_corruption_detected(false), m_recovery_attempts(0) {
	...
	switch (event) {
		...
		case eDVBServicePMTHandler::eventNewProgramInfo: {
			...
			if (m_timeshift_active && scrambling_state_has_changed)
				eDebug("[eDVBServicePlay] Suppressing decoder update.");
			else {
...
void eDVBServicePlay::handleEofRecovery() {
	eDebug("[eDVBServicePlay] Starting recovery process...");
	...
	eDebug("[eDVBServicePlay] Using pre-calculated delay: %lld PTS (~%lld seconds)", m_saved_timeshift_delay, m_saved_timeshift_delay / 90000);

	// Pause the decoder to stabilize the system.
	if (m_decoder) {
		eDebug("[eDVBServicePlay] Pausing decoder.");
		m_decoder->pause();
	}
...
void eDVBServicePlay::onEofRecoveryTimeout() {
	...
	if (m_recovery_attempts++ > 40) {
		eWarning("[eDVBServicePlay] Recovery timed out after 20 seconds. Aborting and unpausing.");
	...
	if (getLength(length) != 0 || getPlayPosition(position) != 0) {
		eWarning("[eDVBServicePlay] Could not get length/position. Retrying in 500ms (Attempt %d)", m_recovery_attempts);
		m_eof_recovery_timer->start(500, true);
		return;
	}
	...
	if ((length - position) < required_buffer) {
		eDebug("[eDVBServicePlay] Waiting for buffer (%llu / %llu)... Retrying in 500ms (Attempt %d)", (length - position), required_buffer, m_recovery_attempts);
	...
	if (m_saved_timeshift_delay > 0 && m_record && !m_record->getCurrentPCR(new_live_pts)) {
		...
		if (new_target_pos < 0) {
			eWarning("[eDVBServicePlay] Calculated target position is negative (%lld). Seeking to beginning.", new_target_pos);
			new_target_pos = 0;
		}

		eDebug("[eDVBServicePlay] Step 1: Attempting to seek to %lld to restore delay.", new_target_pos);
		seekTo(new_target_pos);

		// Step 2: Unpausing directly. The stabilization trick (pause/play) is removed as requested.
		eDebug("[eDVBServicePlay] Step 2: Unpausing decoder.");
		unpause();
	} else {
		// Fallback if we cannot calculate the target position. This will result in a changed delay.
		eWarning("[eDVBServicePlay] Could not get valid delay or live PTS. Falling back to simple unpause.");
		unpause();
	}

	m_stream_corruption_detected = false; // Reset for the next event.
	m_event(this, evSeekableStatusChanged);
	eDebug("[eDVBServicePlay] Recovery complete. Playback should be restored.");
}
	...
	switch (event) {
		...
		case eDVBServicePMTHandler::eventEOF:
			if ((!m_is_paused) && (m_skipmode >= 0)) {
				eDebug("[eDVBServicePlay] EOF event triggered. Initiating safe recovery.");
				handleEofRecovery();
			}
			break;
	}
...
std::string eDVBServicePlay::getInfoString(int w) {
	switch (w) {
		...
		case sVideoInfo: {
			...
			if (m_decoder) {
				char buff[100];
				snprintf(buff, sizeof(buff), "%d|%d|%d|%d|%d|%d", m_decoder->getVideoWidth(), m_decoder->getVideoHeight(), m_decoder->getVideoFrameRate(), m_decoder->getVideoProgressive(),
...
RESULT eDVBServicePlay::getTrackInfo(struct iAudioTrackInfo& info, unsigned int i) {
	...
	const static struct {
		...
		{
			eDVBServicePMTHandler::audioStream::atMPEG,
			"MPEG",
		},
		{
			eDVBServicePMTHandler::audioStream::atAC3,
			"AC3",
		},
		{
			eDVBServicePMTHandler::audioStream::atDDP,
			"AC3+",
		},
		{
			eDVBServicePMTHandler::audioStream::atAC4,
			"AC4",
		},
		{
			eDVBServicePMTHandler::audioStream::atAAC,
			"AAC",
		},
		{
			eDVBServicePMTHandler::audioStream::atDRA,
			"DRA",
		},
		{
			eDVBServicePMTHandler::audioStream::atAACHE,
			"AAC-HE",
		},
		{
			eDVBServicePMTHandler::audioStream::atDTS,
			"DTS",
		},
		{
			eDVBServicePMTHandler::audioStream::atDTSHD,
			"DTS-HD",
		},
		{
			eDVBServicePMTHandler::audioStream::atLPCM,
			"LPCM",
		},
	};
	...
	*/
	if (m_dvb_service && (i != -1 || program.audioStreams.size() == 1 || m_dvb_service->cacheAudioEmpty())) {
		const static struct {
			...
			{
				eDVBAudio::aLPCM,
				eDVBService::cLPCMPID,
			},
			{
				eDVBAudio::aDTSHD,
				eDVBService::cDTSHDPID,
			},
			{
				eDVBAudio::aAACHE,
				eDVBService::cAACHEAPID,
			},
			{
				eDVBAudio::aDRA,
				eDVBService::cDRAAPID,
			},
		};
		static const int nAudioMap = sizeof audioMap / sizeof audioMap[0];
		for (int m = 0; m < nAudioMap; m++) {
...
RESULT eDVBServicePlay::startTimeshift() {
	...
	if (m_timeshift_fd < 0) {
		eDebug("[eDVBServicePlay] mkstemp FAILED for timeshift file. Path is likely invalid or storage is removed. "
			   "Aborting startTimeshift.");
		m_record = 0; // Set recorder pointer to null
...
RESULT eDVBServicePlay::stopTimeshift(bool swToLive) {
	...
	if (m_record) {
		...
		eDebug("[eDVBServicePlay] stopTimeshift called, but no m_record");
	}
...
void eDVBServicePlay::updateTimeshiftPids() {
	...
	if (h.getProgramInfo(program))
		...
		for (std::vector<eDVBServicePMTHandler::videoStream>::const_iterator i(program.videoStreams.begin()); i != program.videoStreams.end(); ++i) {
			if (timing_pid == -1) {
				timing_pid = i->pid;
		...
		for (std::vector<eDVBServicePMTHandler::audioStream>::const_iterator i(program.audioStreams.begin()); i != program.audioStreams.end(); ++i) {
			if (timing_pid == -1) {
				timing_pid = i->pid;
				timing_stream_type = i->type;
		...
		for (std::vector<eDVBServicePMTHandler::subtitleStream>::const_iterator i(program.subtitleStreams.begin()); i != program.subtitleStreams.end(); ++i)
			pids_to_record.insert(i->pid);
		...
		std::set_difference(m_pids_active.begin(), m_pids_active.end(), pids_to_record.begin(), pids_to_record.end(), std::inserter(new_pids, new_pids.begin()));
...
RESULT eDVBServicePlay::getSubtitleList(std::vector<SubtitleTrack>& subtitlelist) {
	...
	if (h.getProgramInfo(program))
		...
		for (std::vector<eDVBServicePMTHandler::subtitleStream>::iterator it(program.subtitleStreams.begin()); it != program.subtitleStreams.end(); ++it) {
			struct SubtitleTrack track = {};
